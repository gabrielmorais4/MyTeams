                            ...
                            client *cli, int sd)
{
    if (cli->team && cli->channel && cli->thread) {
        if (!check_if_uuid_exists(cli->team, "teams", (*serv)->db)) {
            send(sd, CODE_500, strlen(CODE_500) + 1, MSG_NOSIGNAL);
            return 0;
        }
        if (!check_if_uuid_exists(cli->channel, "channels", (*serv)->db)) {
            send(sd, CODE_501, strlen(CODE_501) + 1, MSG_NOSIGNAL);
            return 0;
        }
        if (!check_if_uuid_exists(cli->thread, "threads", (*serv)->db)) {
            send(sd, CODE_502, strlen(CODE_502) + 1, MSG_NOSIGNAL);
            return 0;
        }
                            ...
                            client *cli, int sd)
{
    ...
    if (cli->team && !cli->channel) {
        if (!check_if_uuid_exists(cli->team, "teams", (*serv)->db)) {
            send(sd, CODE_500, strlen(CODE_500) + 1, MSG_NOSIGNAL); return 0;
        }
    ...
    if (cli->team && cli->channel && !cli->thread) {
        ...
        if (!check_if_uuid_exists(cli->channel, "channels", (*serv)->db)) {
            send(sd, CODE_501, strlen(CODE_501) + 1, MSG_NOSIGNAL); return 0;
        }
...
int help_function           (server **serv, client **cli_list,
                            client *current_client, int sd)
{
    send(sd, CODE_200, strlen(CODE_200) + 1, MSG_NOSIGNAL);
    return 0;
}
